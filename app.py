...
def strt(start):
    session = Session(engine)
    start = session.query(func.min(Measurement.tobs),func.avg(Measurement.tobs),func.max(Measurement.tobs)).filter(Measurement.date>=start).all()
    return jsonify (start)
    
@app.route ("/api/v1.0/date/<start_date>/<end_date>")
def st_end(start_date,end_date):

        session = Session(engine)
        st_end = session.query(func.min(Measurement.tobs),func.avg(Measurement.tobs),func.max(Measurement.tobs)).filter(Measurement.date>=start_date).filter(Measurement.date<=end_date).all()
